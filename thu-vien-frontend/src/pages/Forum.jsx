...
const Forum = () => {
    ...
    const userId = userInfo?.id || userInfo?._id;   // ← Chuẩn theo login của bạn
    ...
    const handlePost = async (e) => {
        ...
        if (imageFile) formData.append('image', imageFile);   // ← field name phải là 'image'
    ...
    const handleCommentSubmit = async (postId) => {
        ...
        if (cmtImage) formData.append('image', cmtImage);   // ← ĐÃ SỬA: 'file' → 'image'
                ...
                <div style={createPostCardStyle}>
                    <div style={{ display: 'flex', gap: '15px' }}>
                        ...
                        <textarea
                            ...
                            placeholder={`Bạn đang muốn thảo luận về chủ đề gì?`}
                            style={textareaStyle}
                        />
                    </div>
                    ...
                    <div style={{ borderTop: '1px solid #eee', marginTop: '15px', paddingTop: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <button onClick={handlePost} style={btnStyle} disabled={!newPost.trim() && !imageFile}>
                            🚀 Đăng chủ đề mới
                        </button>
                    </div>
                </div>
                ...
                {posts.length > 0 ? posts.map(post => {
                    ...
                    return (
                        <div key={post._id} style={postCardStyle}>
                            {/* Phần hiển thị bài viết giữ nguyên như cũ, chỉ giữ lại những phần quan trọng */}
                            ...
                            {post.content && <p style={{ fontSize: '15px', lineHeight: '1.5', color: '#1c1e21', marginBottom: '15px', whiteSpace: 'pre-wrap' }}>{post.content}</p>}
                            ...
                            {/* Phần bình luận giữ nguyên logic, chỉ sửa field name đã được xử lý ở handleCommentSubmit */}
                            {expandedComments[post._id] && (
                                <div style={commentSectionStyle}>
                                    {/* Danh sách comment giữ nguyên như code cũ của bạn */}
                                    ...
                                    <div style={{ display: 'flex', gap: '10px', marginTop: '5px', alignItems: 'center' }}>
                                        ...
                                        <div style={{ display: 'flex', flex: 1, backgroundColor: '#f0f2f5', borderRadius: '20px', padding: '5px 15px', alignItems: 'center' }}>
                                            <span style={{cursor: 'pointer', marginRight: '8px', fontSize: '18px'}} onClick={() => setShowEmojis(prev => ({...prev, [post._id]: !prev[post._id]}))}>😀</span>
                                            <input
                                                type="text"

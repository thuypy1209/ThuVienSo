...
const AdminAddBooks = () => {
    ...
    const openCreate = () => {
        setEditing(null);
        setFormData({ title: '', author: '', category: '', description: '', publishedYear: '', price: '', stock: '' });
        setCoverFile(null);
        setPdfFile(null);
        setShowModal(true);
    };
    ...
    return (
        <div className="home-wrapper" style={{ backgroundColor: '#f4f7f6', minHeight: '100vh' }}>
            <nav style={navStyle}>
                ...
                <button onClick={() => navigate('/tu-sach')} style={backBtnStyle}>Quay lại</button>
            </nav>

            <div style={containerStyle}>
                <h2 style={titleStyle}>📚 Quản lý sách</h2>
                {isAdmin && <button onClick={openCreate} style={addBtnStyle}>➕ Thêm sách</button>}
                ...
                <table style={tableStyle}>
                    ...
                    <tbody>
                        {books.map(b => (
                            <tr key={b._id}>
                                <td>
                                    <img src={b.coverImage ? `http://localhost:3000${b.coverImage}` : 'https://via.placeholder.com/60'} width="50" style={{ borderRadius: '6px' }} />
                                </td>
                                <td>{b.title}</td>
                                <td>{b.author}</td>
                                <td style={{ fontWeight: 'bold', color: '#e74c3c' }}>{b.price ? `${b.price.toLocaleString('vi-VN')} ₫` : '—'}</td>
                                <td>{b.stock || '—'}</td>
                ...
                <div style={overlay}>
                    <div style={modalBox}>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            ...
                            <input name="price" type="number" placeholder="Giá tiền (VNĐ)" value={formData.price} onChange={handleChange} required style={inputStyle} />
                            <input name="stock" type="number" placeholder="Số lượng trong kho" value={formData.stock} onChange={handleChange} required style={inputStyle} />

...
const Home = () => {
    ...
    const handleLike = async (bookId) => {
        try {
            await api.post('/wishlists', { book: bookId, user: userInfo._id });
    ...
    return (
        <div className="home-wrapper" style={{ backgroundColor: '#f4f7f6', minHeight: '100vh', paddingBottom: '50px' }}>
            <nav className="navbar" style={navStyle}>
                <div className="nav-logo" style={{ fontSize: '24px', fontWeight: 'bold', color: '#1a5f7a', cursor: 'pointer', whiteSpace: 'nowrap' }} onClick={() => navigate('/home')}>
                    📚 HUTECH Library
                </div>

                <div className="nav-links" style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '15px', fontWeight: '600', color: '#4a4a4a', flex: 1, margin: '0 20px' }}>
                    ...
                    <span style={linkStyle} onClick={() => navigate('/categories')}>
                        📂 Danh mục
                    </span>
                    {(userInfo.role === 'Admin' || userInfo.role === 'Thủ thư') && (
    <span style={linkStyle} onClick={() => navigate('/admin/users')}>
        👥 Quản lý Người Dùng
    </span>
)}
                </div>

                <div className="nav-user" style={{ display: 'flex', alignItems: 'center', gap: '15px', whiteSpace: 'nowrap' }}>
                    ...
                    <div style={gridStyle}>
                        {displayBooks.length > 0 ? displayBooks.map(book => {
                            const imageUrl = book.coverImage
                                ? (book.coverImage.startsWith('http') ? book.coverImage : `http://localhost:3000${book.coverImage}`)
                                ...
                                <div key={book._id} style={cardStyle}>
                                    <div style={thumbContainerStyle}>
                                        ...
                                        <div className="overlay-btn" style={overlayStyle}>
                                            <button onClick={() => navigate(`/doc-sach/${book._id}`)} style={readBtnStyle}>📖 Đọc Ngay</button>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{ padding: '15px' }}>
                                        ...
                                        <p style={{ margin: '0 0 15px 0', color: '#666', fontSize: '13px' }}>
                                            ✍️ {book.author?.name || book.author || "Tác giả ẩn danh"}
                                        </p>
...
const navStyle = { display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', padding: '10px 40px', backgroundColor: 'white', boxShadow: '0 2px 10px rgba(0,0,0,0.08)', position: 'sticky', top: 0, zIndex: 100, gap: '10px' };
